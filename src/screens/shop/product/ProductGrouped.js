...
class ProductGrouped extends React.Component {
  ...
  addToCart = () => {
    ...
    const dataQuantity = dataAdd.filter(
      valueQuantity => valueQuantity.quantity > 0,
    );
  ...
  changeQuantity = (id, valueQuantity) => {
    if (valueQuantity >= 0) {
      const {data} = this.state;
      let {dataAdd} = this.state;
      const getProduct = data.find(p => p.get('id') === id);
      const check = checkQuantity(getProduct.toJS(), valueQuantity);
      if (check) {
